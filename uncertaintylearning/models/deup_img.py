...
class DEUPEstimationImage(BaseModel):
    def __init__(self,
                 data,
        ...
        with torch.no_grad():
            for x, y in ood_loader:
                u_out = torch.clamp(torch.log10(
                    self.loss_fn(self.f_predictor(x.to(self.device)), to_one_hot(y).to(self.device)).sum(1)),
                                    min=-5).view(-1, 1)
                u_in = []
                if 'x' in self.features:
                    epistemic_data.append(x)
                if 'd' in self.features:
                    density_feature = self.density_estimator.score_samples(x, self.device, no_preprocess=False).to(
                        self.device)
                    u_in.append(density_feature)
                if 'v' in self.features:
                    variance_feature = self.variance_source.score_samples(data=x, no_preprocess=False).to(
    ...
    def fit_ood(self, epochs=100, loader=None, classes=None, epistemic_loader=None):
        ...
        if epistemic_loader is None:
            ...
            if 'x' not in self.features:
                self.epistemic_loader = DataLoader(TensorDataset(self.epistemic_X, self.epistemic_Y), shuffle=True,
                                                   batch_size=128)
            else:
                self.epistemic_loader = DataLoader(
                    TensorDataset(self.epistemic_X, self.epistemic_Y, self.epistemic_data), shuffle=True,
                    batch_size=128)
        else:
    ...
    def epistemic_uncertainty(self, x, is_unseen=False):
        ...
        if 'd' in self.features:
            ...
            u_in.append(density_feature)
        if 'v' in self.features:
            variance_feature = self.variance_source.score_samples(data=x, no_preprocess=True).to(self.device).view(-1,
                                                                                                                   1)
            u_in.append(variance_feature)
        if 'b' in self.features:
            if not is_unseen:
