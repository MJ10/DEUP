    ...
    def score_samples(self, test_points, no_preprocess=False):
        ...
        if values.ndim > 0:
            ...
            if torch.is_tensor(values):
                values = values.unsqueeze(-1).unsqueeze(-1)
            else:
                values = torch.FloatTensor(values).unsqueeze(-1).unsqueeze(-1)
        assert values.ndim == 2 and values.size(0) == len(test_points)
        return values
...
class CVKernelDensityEstimator(KernelDensityEstimator):
    ...
    def __init__(self, use_log_density=True, use_density_scaling=False):
        super().__init__(use_log_density, use_density_scaling)
        self.kde = None
        params = {'bandwidth': np.logspace(0, 1, 10), 'kernel': ['gaussian']} # 'exponential', 'tophat', 'gaussian',
